...
namespace ChebsMercenaries.Minions
{
    public class HumanMinion : MercenaryMinion
    {
        ...
        protected IEnumerator WaitForZNet()
        {
            ...
            if (zdo != null && zdo.GetInt("HasCustomEquipment", 0) == 1)
            {
                ...
                if (isSpecialMinion)
                {
                    ...
                    Logger.LogInfo($"Cleared random equipment for special minion {gameObject.name} with weapon {savedWeapon}");
            ...
            humanoid.m_visEquipment.m_currentHelmetItemHash,
                ...
                if (!string.IsNullOrEmpty(weaponName))
                {
                    ...
                    if (weaponPrefab != null)
                    {
                        ...
                        {
                            Logger.LogWarning($"ItemDrop saknas på prefab '{weaponName}'.");
                        }
                    }
                    else
                    {
                    ...
                    if (shieldPrefab != null)
                    {
                        ...
                        if (shieldDrop != null)
                        {
                            ...
                            if (shieldItem != null)
                            {
                                ...
                                Logger.LogInfo($"Utrustade sköld: {shieldName}");
                            }
                            else
                            {
                        ...
                        {
                            Logger.LogWarning($"ItemDrop saknas på prefab '{shieldName}'.");
                        }
                    }
                    else
                    {
                        Logger.LogWarning($"Kunde inte hitta prefab för shield '{shieldName}' i ZNetScene.");
                    }
                }

                // Markera som utrustad om det inte redan är gjort
                zdo.Set("HasCustomEquipment", 1);

                // Anpassa AI om nödvändigt
        ...
        public void ScaleEquipment(MercenaryType mercenaryType, ArmorType armorType, bool useCarapace = false, bool useFlametal = false)
        {
        ...
        private static void ApplyMaterialPropertyBlock(GameObject aiBowPrefab, GameObject targetBowPrefab, string targetName)
        {
        ...
        private static void ApplyPropertyBlockToRenderer(Renderer renderer, VisualProperties properties, string sourceName)
        {
        ...
        private static void ApplySharedMaterialReference(GameObject aiBowPrefab, GameObject targetBowPrefab, string targetName)
        {
            var aiBowRenderers = aiBowPrefab.GetComponentsInChildren<Renderer>(true);
            var targetRenderers = targetBowPrefab.GetComponentsInChildren<Renderer>(true);
        ...
        public static void Spawn(MercenaryType mercenaryType, ArmorType armorType, Transform spawner,
            float chanceOfFemale, List<Vector3> skinColors, List<Vector3> hairColors, bool useCarapace = false, bool useFlametal = false)
        {
